...
namespace Gusev_Revit_PlagIn_Practicum_Lesson3
{
    ...
    public class CopyGroup : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                room = GetRoomByPoint(doc, point);
                roomCenter = GetElementCenter(room);
                groupCenter = roomCenter + offset;

                Transaction transaction = new Transaction(doc);
                transaction.Start("Копирование группу объектов");
                doc.Create.PlaceGroup(groupCenter, group.GroupType);
                transaction.Commit();
            }
